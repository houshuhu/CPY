...
namespace CPy.EntityFrameWork.EfRepository
{
    public class EfRepositoryBase<TEntity, TPrimaryKey> : CPyRepositoryBase<TEntity, TPrimaryKey>
         where TEntity : class ,IAuditEntity<TPrimaryKey>
    {
        ...
        public override void SoftDelete(IEnumerable<TEntity> entities)
        {
        ...
        public override void Update(Expression<System.Func<TEntity, bool>> predicate)
        {
            var entities = GetEntities().Where(predicate);
            Update(entities);
        }
